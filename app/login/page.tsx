...
import { Dumbbell, Mail, Lock, Loader2, ArrowLeft, Send, MessageCircle } from 'lucide-react';
  ...
  const handleRecoverPassword = async (method: 'email' | 'whatsapp') => {
    ...
    try {
      if (method === 'email') {
        ...
        const { data, error } = await supabase.from('profiles').select('telefone').eq('email', email).maybeSingle();
        if (error || !data?.telefone) {
          throw new Error('Número de WhatsApp não encontrado para este e‑mail.');
        }
      ...
      showToast(err.message || 'Falha ao processar solicitação.', 'error');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white dark:bg-[#12141c] p-8 rounded-3xl shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#1e2235] relative overflow-hidden">
          {!isRecovering ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="login-id" className="block text-[11px] uppercase tracking-widest font-bold text-gray-500 dark:text-gray-400 mb-2">
                  E‑mail ou Celular
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    aria-describedby={fieldError.email ? 'login-id-error' : undefined}
                  />
                </div>
                {fieldError.email && (
                  <p id="login-id-error" className="mt-1 text-sm text-red-600 dark:text-red-400">
                    {fieldError.email}
                  </p>
                )}
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label htmlFor="password" className="block text-[11px] uppercase tracking-widest font-bold text-gray-500 dark:text-gray-400">
                    Senha
                  </label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-[#1a1d27] border border-gray-200 dark:border-[#2d3348] rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-5 outline-none transition-all"
                    placeholder="••••••••"
